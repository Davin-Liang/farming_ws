...
class Game_Controller(Node):
    def __init__(self, name):
        ...
        self.ori_angle_pid = PID(Kp=0.698, Ki=0.0003, Kd=0.456, max_out=0.125, max_iout=0.0065)
...
# -----------------------------------------------------------------------------------------------------------------------------
    def auto_pollinate(self, 
                       place="A", 
                       arm_pose="a_left", 
                       joint2_area_threthold=38000, 
                       joint2_coefficient=1.5,
                       area_of_polliating=70000):
        ...
        if "A" == place or "C" == place:
            ...
            if self.error:
                if self.new_choice:
                    self.guo_xiaoyu_is_broadcasting("One more time!!!!!!")
    ...
    def car_action_in_lidar(self, 
                            speed, 
                            threthold=0.1, 
                            distance_threshold=0.25):
        """ Start car and stop car by lidar. """
        while abs(self.angle-self.yaw_angle) > self.angle_tolerance:
            self.guo_xiaoyu_is_broadcasting("Wating for returning to the correct attitude.")
    ...
    def choose_arm_goal_in_number(self, 
                                  joint1=0, 
                                  joint2=0, 
                                  joint3=0, 
                                  joint4=0, 
...
# -----------------------------------------------------------------------------------------------------------------------------
                ...
                for index, flower_with_tag in enumerate(self.flowers_with_tag):
                    if flower_with_tag['Moving'] == True:
                        if self.calculate_O_distance_(flower_with_tag['CentralPoint'], flower['CentralPoint']) < self.O_distance_threthold_of_judge_same_goal:
                            self.guo_xiaoyu_is_broadcasting("Satisfy the O-type distance limit!!!!!!")
                            if abs(flower_with_tag['Area']-flower['Area']) <= self.area_difference:
                                self.guo_xiaoyu_is_broadcasting("Meet the area change limit!!!!!!")
    ...
    def data_pre_processing_(self, flowers_lists):
        """ Adds flower attributes to the dictionary that stores flower attributes: 
        whether it is being operated, 
        whether it has been pollinated """
        flower_with_tag = {'Type': '', 'CentralPoint': [], 'Area': 0, 'Moving': False, 'Pollinated': False}
        if self.pre_process:
            if len(self.flowers_with_tag) == 0:
                print("Data preprocessing is underway!!!!!!")
                ...
                self.guo_xiaoyu_is_broadcasting("Pollinating the next target point!!!!!!")
    ...
    def control_arm_(self):
        ...
        if self.place_name == 'A':
            if area < self.joint2_area_threthold:                                    
    ...
    def reset_arm_pose_(self, pose="a_left"):
        """ Control arm back to initial attitude. """
    ...
    def find_closest_flower(self, flowers_lists, flowers_with_tag):
        """ Find the flower with the smallest CentralPoint difference from the specified flower. """
    ...
    def calculate_O_distance_(self, point1, point2):
        """ Calculate the O-distance between two points. """
...
# -----------------------------------------------------------------------------------------------------------------------------
    ...
    def arm_timer_callback_(self):
        ...
        if self.reset_count > self.reset_count_threshold:
            self.error = True
            self.voice_state = True
            self.guo_xiaoyu_is_broadcasting("Target point loss!!!!!!")
            self.guo_xiaoyu_is_broadcasting("Target point loss!!!!!!")
            self.guo_xiaoyu_is_broadcasting("Target point loss!!!!!!")
            self.reset_arm_pose_(self.pose_name)
        else:
