...
class Motion_Controller(Node):
    ...
    def get_position_(self):
        try:
            ...
            trans = self.tf_buffer.lookup_transform(self.odom_frame, self.base_frame, now, Duration(seconds=5.0))   
            return trans       
    ...
    def get_odom_angle_(self):
        ...
        try:
            now = rclpy.time.Time()
            rot = self.tf_buffer.lookup_transform(self.odom_frame, self.base_frame, now, Duration(seconds=5.0))
            # 创建了一个四元数对象   
